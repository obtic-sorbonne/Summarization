'''This script is designed to extract keywords from an article in XML format.

Author: Johanna Cordova
Contributor: James Gawley
...
if len(sys.argv) < 2:
	...
	filepath = sys.argv[1]
	outpath = filepath.replace("xml/", "keys_")
	outpath = outpath.replace(".xml", ".txt")

	"_".join(["keys", sys.argv[1]])
...
outfile.write("Title: {}".format(title))
outfile.write("\n")

# Nombre de mots
nb_words = count_words(soup)
outfile.write("Number of words : {}".format(nb_words))
outfile.write("\n")
...
for sec in sections:
	...
	outfile.write("Section title : {}".format(current_sec_title))
	outfile.write("\n")
	...
	keywords_MMR = [tup for tup in keywords_MMR if tup[1] > .5]
	outfile.write("Keywords with MMR: {}\n".format(keywords_MMR))
	...
	keywords_MSD = [tup for tup in keywords_MSD if tup[1] > .5]
	outfile.write("Keywords with MSD: {}\n".format(keywords_MSD))

	overview[current_sec_title] = keywords_MSD
	outfile.write("\n")
outfile.close()
writeHTML(filepath, title, nb_words, m, "No summary.", overview, intro_summary, conclusion_summary)